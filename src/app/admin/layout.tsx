...
import { 
  ...
  ShieldCheck, HelpCircle // Tambahkan HelpCircle
} from "lucide-react";
...
const sidebarLinks = [
  ...
  { name: "FAQ Manager", href: "/admin/faqs", icon: HelpCircle }, // Link Baru di sini
    ...
    <div className="flex min-h-screen bg-[#020617] text-slate-200 font-sans selection:bg-teal-500/30">
      ...
      <aside className="hidden lg:flex flex-col w-72 bg-slate-900/40 border-r border-white/5 backdrop-blur-xl sticky top-0 h-screen p-6">
        ...
        <div className="flex items-center gap-3 mb-12 px-2">
          <div className="w-10 h-10 bg-teal-400 rounded-2xl flex items-center justify-center shadow-lg shadow-teal-500/20">
        ...
        <nav className="flex-1 space-y-2">
          {sidebarLinks.map((item) => {
            // Logic Active Link yang lebih presisi
            ...
            return (
              <Link 
                ...
                className={`flex items-center justify-between group px-4 py-3.5 rounded-2xl transition-all border ${
                  isActive 
                  ? "bg-teal-400/10 text-teal-400 border-teal-400/20 shadow-[0_0_15px_rgba(45,212,191,0.05)]" 
        ...
        <div className="mt-auto pt-6 border-t border-white/5">
          <div className="flex items-center gap-3 px-4 py-3 bg-white/5 rounded-2xl border border-white/5 group hover:border-teal-400/20 transition-colors">
            <ShieldCheck size={16} className="text-teal-400/50 group-hover:text-teal-400 transition-colors" />
            <span className="text-[9px] font-black uppercase tracking-[0.2em] text-white/20 group-hover:text-white/40 transition-colors">Secure Session</span>
          </div>
        </div>
      </aside>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-[#020617]/80 backdrop-blur-md border-b border-white/5 p-4 flex justify-between items-center">
        ...
        <button onClick={() => setIsMobileOpen(true)} className="p-2 bg-white/5 rounded-xl text-teal-400 transition-transform active:scale-90">
          <Menu size={20} />
        </button>
      </div>
      ...
      <main className="flex-1 w-full lg:max-w-[calc(100%-18rem)] pt-20 lg:pt-0">
        <div className="h-full min-h-screen relative">
          {/* Neon Gradients */}
          <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-teal-400/5 blur-[120px] -z-10 rounded-full animate-pulse" />
        ...
        {isMobileOpen && (
          <>
            ...
            <motion.aside 
              ...
              className="fixed top-0 left-0 z-[70] w-4/5 h-full bg-[#020617] p-8 border-r border-white/10 lg:hidden flex flex-col shadow-2xl"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex justify-between items-center mb-10">
                <span className="font-black italic text-teal-400 uppercase tracking-widest text-xs">Menu Control</span>
                <button onClick={() => setIsMobileOpen(false)} className="text-white/40 hover:text-white transition-colors"><X size={24} /></button>
              </div>
                  ...
                  return (
                    <Link 
                      ...
                      className={`flex items-center gap-4 text-xs font-bold uppercase tracking-[0.2em] py-4 px-4 rounded-xl border transition-all ${
                        isActive ? "bg-teal-400/10 text-teal-400 border-teal-400/20 shadow-lg shadow-teal-400/5" : "text-white/40 border-transparent"
                      }`}
                    >
              ...
              <div className="mt-auto">
                 <p className="text-[8px] font-black uppercase tracking-[0.4em] text-white/10 text-center italic">
